...
namespace NetworkOfAirports_EF.DAL.Expansion.GenerateData
{
    public static class DataGenerator
    {
        ...
        public static void Generate()
        {
            ...
            for (int i = 0; i < FLIGHT_PASSENGERS; i++)
            {
                var temp = new Faker<FlightPassenger>()
                    .RuleFor(x => x.FlightId, f => f.PickRandom(Flights).FlightId)
                    .RuleFor(x => x.PassengerId, f => f.PickRandom(Passengers).PassengerId)
                    .Generate(1);
                if (!FlightPassengers.Any(x => x.FlightId == temp[0].FlightId && x.PassengerId == temp[0].PassengerId))
                {
                    FlightPassengers.Add(temp[0]);
                }
                else
                {
                    i--;
